# Run model across the range of observed values for depensation
rm(list = ls())
...
dep_whi_other <- simulate_along(par_range = dep_range, params = param_vec, par_id = 1, utilfun = Pi_whi_other, Emax = 48, Bmsy = 0.00001436516, utilname = "whi_other")
dep_whi_mackerel <- simulate_along(par_range = dep_range, params = param_vec, par_id = 1, utilfun = Pi_whi_mackerel, Emax = 48, Bmsy = 0.00001455438, utilname = "whi_mackerel")
...
scaled_dat_whi_other <- scale_output(dat = dep_whi_other, ref_param_val = 0.001)
scaled_dat_whi_mackerel <- scale_output(dat = dep_whi_mackerel, ref_param_val = 0.001)
...
dep_kur_hms_plot <- outvar_heatmap(dat = scaled_dat_kur_hms, 
                                   ref = 0.001,
                                 title = "High intercept, low steepness, Emax = 38",
                                 dat_range = dat_range,
                                 emp_dat= emp_dep,
...
dep_kur_bottom_plot <- outvar_heatmap(dat = scaled_dat_kur_bottom, 
                                      ref = 0.001,
                                    title = "High intercept, high steepness, Emax = 25",
                                    dat_range = dat_range,
                                    emp_dat= emp_dep,
                                    xlab = NULL,
                                    ylabelling = FALSE)
...
dep_rag_prize_plot <- outvar_heatmap(dat = scaled_dat_rag_prize, 
                                     ref = 0.001,
                              title = "Low intercept, low steepness, Emax = 696",
                              dat_range = dat_range,
                              emp_dat= emp_dep,
...
dep_whi_sg_plot <- outvar_heatmap(dat = scaled_dat_whi_sg, 
                                  ref = 0.001,
                                title = "Low intercept, high steepness, Emax = 21",
                                dat_range = dat_range,
                                emp_dat= emp_dep,
                                xlab = "Depensation parameter d",
                                ylabelling = FALSE)

dep_whi_other_plot <- outvar_heatmap(dat = scaled_dat_whi_other, 
                                  ref = 0.001,
                                  title = "Median intercept, low steepness, Emax = 92",
                                  dat_range = dat_range,
                                  emp_dat= emp_dep,
                                  xlab = "Depensation parameter d")

dep_whi_mackerel_plot <- outvar_heatmap(dat = scaled_dat_whi_mackerel, 
                                  ref = 0.001,
                                  title = "Median intercept, high steepness, Emax = 36",
                                  dat_range = dat_range,
                                  emp_dat= emp_dep,
                                  xlab = "Depensation parameter d",
                                  ylabelling = FALSE)
...
figname <- paste(todaysdate, "dep_plot_Emax_Emsy_function_specific.png", sep = "-")
png(paste(outfig, figname, sep = "/"), width = 12, height =10, units = "in", res = 1000)

(dep_kur_hms_plot + dep_kur_bottom_plot) / (dep_whi_other_plot + dep_whi_mackerel_plot) / (dep_rag_prize_plot + dep_whi_sg_plot) +plot_layout(guides = "collect") + 
  plot_annotation("Emax = Emsy for each function")
...
figname <- paste(todaysdate, "figS1_dep_sensitivity_analysis.png", sep = "-")
png(paste(outfig, figname, sep = "/"), width = 12, height = 10, units = "in", res = 1000)
...
figname <- paste(todaysdate, "figS1_dep_sensitivity_analysis_for_ppt.png", sep = "-")
