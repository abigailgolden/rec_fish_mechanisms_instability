...
sd_plot <- outvar_heatmap(dat = sd_scaled, 
                           ref = 0,
                           title = "B) SD of recruitment stochasticity",
                           dat_range = dat_range,
                           emp_dat= emp_sd,
...
rho_plot <- outvar_heatmap(dat = rho_scaled,
                           ref = 0,
                           title = "C) Autocorrelation in recruitment stochasticity",
                           dat_range = dat_range,
                           emp_dat= emp_rho,
...
hyp_plot <- outvar_heatmap(dat = hyp_scaled,
                           title = "D) Density dependence in catchability",
                           ref = 1,
                           dat_range = dat_range,
                           emp_dat= emp_b,
...
figname <-  paste(todaysdate, "fig4.png", sep = "_")
png(paste(outfig, figname, sep = "/"), width = 14, height = 10, units = "in", res = 600)

(dep_plot + sd_plot) / (rho_plot + hyp_plot) + plot_layout(guides = "collect")
